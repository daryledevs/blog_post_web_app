...
async function up(db) {
    ...
    await db.schema
        ...
        .modifyColumn("follower_id", "integer", (col) => col.notNull().unsigned())
        .modifyColumn("followed_id", "integer", (col) => col.notNull().unsigned())
        .modifyColumn("created_at", "timestamp", (col) => col.defaultTo((0, kysely_1.sql) `CURRENT_TIMESTAMP`))
        .execute()
    ...
    await db.schema
        ...
        .execute()
        .then(() => console.log("Created index idx_members_conversation_id on 'conversation_members' table"));
    await db.schema
        .createIndex("idx_members_user_id")
        .on("conversation_members")
        .column("user_id")
        .execute()
        .then(() => console.log("Created index idx_members_user_id on 'conversation_members' table"));
    ...
    await db.schema
        ...
        .onDelete("cascade")
        .execute()
        .then(() => console.log("Added foreign key 'fk_messages_conversation_id' with ON DELETE CASCADE to 'messages' table"));
    await db.schema
        .createIndex("idx_messages_conversation_id")
        .on("messages")
        .column("conversation_id")
        .execute()
        .then(() => console.log("Created index idx_messages_conversation_id on 'messages' table"));
    await db.schema
        .alterTable("messages")
        .addForeignKeyConstraint("fk_sender_id", ["sender_id"], "users", ["id"])
        .onDelete("cascade")
        .execute()
        .then(() => console.log("Added foreign key fk_messages_user_id with ON DELETE CASCADE to 'messages' table"));
    await db.schema
