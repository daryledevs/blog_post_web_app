...
async function up(db) {
    ...
    await db.schema
        ...
        .execute()
        .then(() => console.log("Dropped constraint reset_token_user_id_fk successfully"));
    await db.schema
        .alterTable("followers")
        .dropConstraint("follower_user_id_fk")
        .execute()
        .then(() => console.log("Dropped constraint follower_user_id_fk successfully"));
    await db.schema
        .alterTable("followers")
        .dropConstraint("followed_user_id_fk")
        .execute()
        .then(() => console.log("Dropped constraint followed_user_id_fk successfully"));
    await db.schema
    ...
    await db.schema
        ...
        .execute()
        .then(() => console.log("Dropped column 'comment_id' from 'comments' table"));
    await db.schema
        .alterTable("comments")
        .dropConstraint("comment_post_id_fk")
        .execute()
        .then(() => console.log("Dropped constraint 'comment_post_id_fk' from 'comments' table"));
    await db.schema
        .alterTable("comments")
        .dropConstraint("comment_user_id_fk")
        .execute()
        .then(() => console.log("Dropped constraint 'comment_user_id_fk' from 'comments' table"));
    await db.schema
        .alterTable("likes")
        .dropConstraint("user_id_likes_fk")
        .execute()
        .then(() => console.log("Dropped user_id_likes_fk constraint successfully"));
    await db.schema
        .alterTable("likes")
        .dropConstraint("post_id_likes_fk")
        .execute()
        .then(() => console.log("Dropped post_id_likes_fk constraint successfully"));
    await db.schema
        .alterTable("posts")
        .dropColumn("post_id")
        .execute()
    ...
    await db.schema
        ...
        .execute()
        .then(() => console.log("Dropped column 'message_id' from 'messages' table"));
    await db.schema
        .alterTable("messages")
        .dropConstraint("message_conversation_id_fk")
        .execute()
        .then(() => console.log("Dropped constraint 'message_conversation_id_fk' from 'messages' table"));
    await db.schema
        .alterTable("messages")
        .dropConstraint("message_sender_id_fk")
        .execute()
        .then(() => console.log("Dropped constraint 'message_sender_id_fk' from 'messages' table"));
    await db.schema
