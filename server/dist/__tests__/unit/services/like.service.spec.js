...
(0, vitest_1.describe)("LikeService", () => {
    let cloudinary;
    let likeRepository;
    let postRepository;
    let userRepository;
    ...
    (0, vitest_1.describe)("getPostLikesCountByUuid (get the total likes for the post)", async () => {
        (0, vitest_1.test)("should return the correct result", async () => {
            ...
            const result = await likeService.getPostLikesCountByUuid(existingPostDto.getUuid());
            ...
            (0, vitest_1.expect)(likeRepository.findPostsLikeCount).toHaveBeenCalledWith(existingPostDto.getId());
        });
        (0, vitest_1.test)("should throw an error if post not found", async () => {
            ...
            await (0, vitest_1.expect)(likeService.getPostLikesCountByUuid(nonExistingPostDto.getUuid())).rejects.toThrow(error.postNotFoundMsg);
    ...
    (0, vitest_1.describe)("getUserLikeStatusForPostByUuid (check if the user liked the post)", async () => {
        (0, vitest_1.test)("should return the correct result", async () => {
            const like = generate_data_util_1.default.createLike(existingPostDto.getUuid(), existingPostDto.getUuid());
            userRepository.findUserById = vitest_1.vi.fn().mockResolvedValue(existingUserDto);
            postRepository.findPostsByPostId = vitest_1.vi.fn().mockResolvedValue(existingPostDto);
            likeRepository.isUserLikePost = vitest_1.vi.fn().mockResolvedValue(like);
            const result = await likeService.getUserLikeStatusForPostByUuid(existingUserDto.getUuid(), existingPostDto.getUuid());
            ...
            (0, vitest_1.expect)(postRepository.findPostsByPostId).toHaveBeenCalledWith(existingPostDto.getUuid());
            (0, vitest_1.expect)(likeRepository.isUserLikePost).toHaveBeenCalledWith(existingUserDto.getUuid(), existingPostDto.getUuid());
        });
        (0, vitest_1.test)("should throw an error if user not found", async () => {
            ...
            await (0, vitest_1.expect)(likeService.getUserLikeStatusForPostByUuid(notFoundUserDto.getUuid(), existingPostDto.getUuid())).rejects.toThrow(error.userNotFoundMsg);
        ...
        (0, vitest_1.test)("should throw an error if post not found", async () => {
            ...
            postRepository.findPostsByPostId = vitest_1.vi.fn().mockResolvedValue(undefined);
            likeRepository.isUserLikePost = vitest_1.vi.fn();
            await (0, vitest_1.expect)(likeService.getUserLikeStatusForPostByUuid(existingUser.uuid, nonExistingPostDto.getUuid())).rejects.toThrow(error.postNotFoundMsg);
    ...
    (0, vitest_1.describe)("toggleUserLikeForPost (toggle the user's like for the post)", async () => {
        (0, vitest_1.test)("should return the 'like added successfully' message", async () => {
            ...
            const result = await likeService.toggleUserLikeForPost(existingUser.uuid, existingPostDto.getUuid());
            (0, vitest_1.expect)(result).toBe(expectedMsg);
            (0, vitest_1.expect)(userRepository.findUserById).toHaveBeenCalledWith(existingUser.uuid);
            (0, vitest_1.expect)(postRepository.findPostsByPostId).toHaveBeenCalledWith(existingPostDto.getUuid());
            (0, vitest_1.expect)(likeRepository.isUserLikePost).toHaveBeenCalledWith(existingUser.id, existingPostDto.getUuid());
        ...
        (0, vitest_1.test)("should throw an error if user not found", async () => {
            ...
            likeRepository.likeUsersPostById = vitest_1.vi.fn();
            likeRepository.dislikeUsersPostById = vitest_1.vi.fn();
            await (0, vitest_1.expect)(likeService.toggleUserLikeForPost(notFoundUser.uuid, existingPostDto.getUuid())).rejects.toThrow(error.userNotFoundMsg);
            (0, vitest_1.expect)(userRepository.findUserById).toHaveBeenCalledWith(notFoundUser.uuid);
            (0, vitest_1.expect)(postRepository.findPostsByPostId).not.toHaveBeenCalled();
            (0, vitest_1.expect)(likeRepository.isUserLikePost).not.toHaveBeenCalled();
            (0, vitest_1.expect)(likeRepository.likeUsersPostById).not.toHaveBeenCalled();
