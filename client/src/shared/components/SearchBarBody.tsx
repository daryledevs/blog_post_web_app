...
function SearchBarBody({ search, setClickedLink, setSearch }: SearchBarBodyProps) {
  const userApiData = useGetUserDataQuery({ person: "" });
  const searchesApiData = useSearchUsersQuery({ search }, { skip: !search });
  const [saveRecentSearch, saveRecentSearchApi] = useSaveRecentSearchUserMutation();
  const [deleteRecentSearch, deleteRecentSearchApi] = useDeleteRecentSearchUserMutation();
  ...
  if (searchesApiData.isLoading || recentSearchesDataApi.isLoading) return <></>;
    ...
    <div className="search-body__container">
      <div className="search-body__list">
        <SearchBarUserList
          ...
          onClick={search ? saveRecentSearch : deleteRecentSearch}
        />
      </div>
    </div>
  );
